...
/// ShareTemplateCard - Card 模板独立模块
///
/// 现代深度卡片设计，包含多层阴影效果、渐变装饰条和优雅的排版
class ShareTemplateCard {
  // 图片缓存 - 避免重复加载
  ...
  /// 绘制卡片模板 - 现代深度卡片设计
  static Future<void> drawCardTemplate(
    Canvas canvas,
    Size size,
    String content,
    DateTime timestamp,
  ...
  static List<TextSpan> _parseMarkdownToSpans(
    ...
    if (kDebugMode) {
      debugPrint(
        'ShareTemplateCard: 开始解析Markdown内容: "${content.substring(0, content.length > 50 ? 50 : content.length)}..."',
      );
    }
    ...
    for (final match in matches) {
      ...
      if (match.start > lastIndex) {
        ...
        if (plainText.isNotEmpty) {
          if (kDebugMode) {
            debugPrint(
              'ShareTemplateCard: 解析普通文本: "${plainText.length > 20 ? plainText.substring(0, 20) : plainText}..."',
            );
          }
          spans.addAll(
      ...
      if (kDebugMode) debugPrint('ShareTemplateCard: 解析到标签: "$tagText"');
    ...
    if (lastIndex < content.length) {
      ...
      if (remainingText.isNotEmpty) {
        if (kDebugMode) {
          debugPrint(
            'ShareTemplateCard: 解析剩余文本: "${remainingText.length > 20 ? remainingText.substring(0, 20) : remainingText}..."',
          );
        }
        spans.addAll(
    ...
    if (kDebugMode) {
      debugPrint('ShareTemplateCard: 解析完成，共生成 ${spans.length} 个span');
    }
    return spans;
  }
    ...
    for (var lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      if (line.startsWith('#')) {
        ...
        if (titleMatch != null) {
          ...
          if (kDebugMode) {
            debugPrint('ShareTemplateCard: 解析到H$level标题: "$titleText"');
          }
      ...
      if (char == '*' && i + 1 < text.length) {
        ...
        if (text[i + 1] == '*') {
          ...
          if (endIndex != -1) {
            ...
            if (kDebugMode) {
              debugPrint('ShareTemplateCard: 解析到粗体: "$boldText"');
            }
          ...
          if (endIndex != -1) {
            ...
            if (kDebugMode) {
              debugPrint('ShareTemplateCard: 解析到斜体: "$italicText"');
            }
        ...
        if (endIndex != -1) {
          ...
          if (kDebugMode) {
            debugPrint('ShareTemplateCard: 解析到代码: "$codeText"');
          }
  ...
  static void clearImageCache() {
    _imageCache.clear();
    _cacheSize = 0;
    if (kDebugMode) debugPrint('ShareTemplateCard: 图片缓存已清理');
  }
