...
class _DataCleanupScreenState extends State<DataCleanupScreen> {
  ...
  Future<void> _loadDataInfo() async {
    ...
    try {
      ...
      // 获取图片数量
      final imagesCount = await _getImagesCount();
  ...
  Future<void> _clearAllNotes() async {
    final confirmed = await _showConfirmDialog(
      AppLocalizationsSimple.of(context)?.cleanupConfirm ?? '确认清理',
  ...
  Future<void> _resetAppSettings() async {
    final confirmed = await _showConfirmDialog(
      AppLocalizationsSimple.of(context)?.confirmReset ?? '确认重置',
        ...
        builder: (context) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: Text(AppLocalizationsSimple.of(context)?.confirm ?? '确认'),
            ),
          ],
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizationsSimple.of(context)?.dataCleanup ?? '数据清理'),
        elevation: 0,
  ...
  // 数据统计卡片
  Widget _buildStatisticsCard(Color cardColor, Color textColor) => Card(
        elevation: 1,
        color: cardColor,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                AppLocalizationsSimple.of(context)?.dataStatistics ?? '数据统计',
                style: TextStyle(
                  fontSize: 18,
              ...
              _buildStatItem(
                Icons.storage,
                AppLocalizationsSimple.of(context)?.databaseSize ?? '数据库大小',
                '${_databaseSize.toStringAsFixed(2)} MB',
              ),
              const SizedBox(height: 8),
              _buildStatItem(
                Icons.cached,
                AppLocalizationsSimple.of(context)?.cacheSize ?? '缓存大小',
                '${_cacheSize.toStringAsFixed(2)} MB',
              ),
              const SizedBox(height: 8),
              _buildStatItem(
                Icons.image,
                AppLocalizationsSimple.of(context)?.imagesCount ?? '图片数量',
                '$_imagesCount ${AppLocalizationsSimple.of(context)?.imagesItem ?? '张图片'}',
              ),
              ...
              Center(
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    AppLocalizationsSimple.of(context)?.refreshData ?? '刷新数据',
                  ),
                  style: ElevatedButton.styleFrom(
                    padding:
  ...
  // 统计项
  Widget _buildStatItem(IconData icon, String label, String value) => Row(
        children: [
  ...
  // 清理操作卡片
  Widget _buildCleanupActionsCard(Color cardColor, Color textColor) => Card(
        elevation: 1,
        color: cardColor,
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(12),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                AppLocalizationsSimple.of(context)?.cleanupOperations ?? '清理操作',
                style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                  color: textColor,
              ...
              _buildActionItem(
                ...
                title: AppLocalizationsSimple.of(context)?.cleanCache ?? '清理缓存',
                subtitle:
                    AppLocalizationsSimple.of(context)?.cleanCacheDescription ??
                        '删除临时文件和缓存，不会影响笔记数据',
              ...
              _buildActionItem(
                ...
                title: AppLocalizationsSimple.of(context)?.cleanUnusedImages ??
                    '清理未使用图片',
  ...
  // 高级操作卡片
  Widget _buildAdvancedActionsCard(Color cardColor, Color textColor) => Card(
        elevation: 1,
        color: cardColor,
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(12),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                AppLocalizationsSimple.of(context)?.advancedOperations ??
                    '高级操作',
                style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
              ...
              _buildActionItem(
                ...
                title: AppLocalizationsSimple.of(context)?.resetAppSettings ??
                    '重置应用设置',
                subtitle: AppLocalizationsSimple.of(context)
                        ?.resetSettingsDescription ??
                    '将所有应用设置恢复到默认状态，不会删除笔记数据',
              ...
              _buildActionItem(
                ...
                title: AppLocalizationsSimple.of(context)?.deleteAllNotes ??
                    '清理所有笔记',
                subtitle: AppLocalizationsSimple.of(context)
                        ?.dangerOperationSubtitle ??
                    '危险操作：删除所有本地笔记数据，此操作不可恢复',
